...
let interpret : annot_value -> [> typed_value] =
  ...
  | Some "i8", `Int i -> failwith @@ sprintf "%d is not a valid i8" i
  | Some "i8", _ -> failwith "i8 expects an integer value"

  | Some "i16", `Int i when i >= -0x80_00 && i < 0x80_00 -> `I16 i
  | Some "i16", `Int i -> failwith @@ sprintf "%d is not a valid i16" i
  | Some "i16", _ -> failwith "i16 expects an integer value"

  | Some "i32", `Int i when Sys.int_size >= 33 ->
    if i >= min_int32 && i <= max_int32 then
      `I32 (Int32.of_int i)
    else
      failwith @@ sprintf "%d is not a valid i32" i
  | Some "i32", `Int i -> `I32 (Int32.of_int i)
  | Some "i32", `RawInt istr -> (match Int32.of_string_opt istr with
    | Some i -> `I32 i
    | None -> failwith @@ sprintf "%s is not a valid i32" istr)
  | Some "i32", _ -> failwith "i32 expects an integer value"

  | Some "i64", `Int i -> `I64 (Int64.of_int i)
  | Some "i64", `RawInt istr -> (match Int64.of_string_opt istr with
    | Some i -> `I64 i
    | None -> failwith @@ sprintf "%s is not a valid i64" istr)
  | Some "i64", _ -> failwith "i64 expects an integer value"

  | Some "u8", `Int i when i >= 0 && i <= 0xff -> `U8 i
  | Some "u8", `Int i -> failwith @@ sprintf "%d is not a valid u8" i
  | Some "u8", _ -> failwith "u8 expects an integer value"

  | Some "u16", `Int i when i >= 0 && i <= 0xff_ff -> `U16 i
  | Some "u16", `Int i -> failwith @@ sprintf "%d is not a valid u16" i
  | Some "u16", _ -> failwith "u16 expects an integer value"

  | Some "u32", `Int i when Sys.int_size >= 33 ->
    if i >= 0 && i <= max_uint32 then
      `U32 (Int32.of_int i)
    else
      failwith @@ sprintf "%d is not a valid u32" i
  | Some "u32", `Int i ->
    (* Note that [i] can be negative *)
    `U32 (Int32.of_int i)
  | Some "u32", `RawInt istr -> (match Int32.of_string_opt (to_unsigned istr) with
    | Some i -> `U32 i
    | None -> failwith @@ sprintf "%s is not a valid u32" istr)
  | Some "u32", _ -> failwith "u32 expects an integer value"

  | Some "u64", `Int i -> `U64 (Int64.of_int i)
  | Some "u64", `RawInt istr -> (match Int64.of_string_opt (to_unsigned istr) with
    | Some i -> `U64 i
    | None -> failwith @@ sprintf "%s is not a valid u64" istr)
  | Some "u64", _ -> failwith "u64 expects an integer value"

  | Some "isize", `Int i -> `Isize (Nativeint.of_int i)
  | Some "isize", `RawInt istr -> (match Nativeint.of_string_opt istr with
    | Some i -> `Isize i
    | None -> failwith @@ sprintf "%s is not a valid isize" istr)
  | Some "isize", _ -> failwith "isize expects an integer value"

  | Some "usize", `Int i -> `Usize (Nativeint.of_int i)
  | Some "usize", `RawInt istr -> (match Nativeint.of_string_opt (to_unsigned istr) with
    | Some i -> `Usize i
    | None -> failwith @@ sprintf "%s is not a valid usize" istr)
  | Some "usize", _ -> failwith "usize expects an integer value"

  (* There is currently no difference between f32 and f64 *)
  | Some "f32", `Float f -> `F32 f
  | Some "f32", _ -> failwith "f32 expects a float value"

  | Some "f64", `Float f -> `F64 f
  | Some "f64", _ -> failwith "f64 expects a float value"

  | Some "base64", `String base64 -> `Base64 (Base64.decode base64)
  | Some "base64", _ -> failwith "base64 expects a string value"
